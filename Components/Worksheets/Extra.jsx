...
import { sql_extra } from '../../Data/worksheets'
import DynamicTable from '../Worksheets/results'
import { keywords } from '../../Data/worksheets'
import { CSVLink } from 'react-csv';


function Extra({ database, changeDB }) {


    const [data, setData] = useState(sql_extra.results || []);

    const [is_run, run_sql] = useState(false);
    const [isOpen, setIsOpen] = useState(true);

    const [cols, setCols] = useState(Object.keys(sql_extra.results[0]).reduce((acc, col) => {
    ...
    useEffect(() => {
        ...
        if (searchRow && searchRow.length > 0) {
            ...
            setData(sql_extra.results.filter((row) =>
    ...
    console.log(is_run);
    const sql_words = sql_extra.query.split(" ");
    const count = 1;
            ...
            <div className="row mb-1">
                ...
                {isOpen && <div className="col-6 d-flex align-items-center justify-content-end text-secondary">
                    ...
                    >
                        ...
                        {
                            Object.keys(sql_extra.results[0]).map((col) => (
