...
import { sql_contact } from '../../Data/worksheets'
import DynamicTable from './results'
import { keywords } from '../../Data/worksheets'
import { CSVLink } from 'react-csv';


function Contact({ database, changeDB }) {


    const [data, setData] = useState(sql_contact.results || []);

    const [is_run, run_sql] = useState(false);
    const [isOpen, setIsOpen] = useState(true);

    const [cols, setCols] = useState(Object.keys(sql_contact.results[0]).reduce((acc, col) => {
    ...
    useEffect(() => {
        ...
        if (searchRow && searchRow.length > 0) {
            ...
            setData(sql_contact.results.filter((row) =>
    ...
    console.log(is_run);
    const sql_words = sql_contact.query.split(" ");
    const count = 1;

    const worksheetProps = {
        database: database,
        file: sql_contact.file,
        changeDB: changeDB,
        run_sql: run_sql
    };
            ...
            <div className="row mb-1">
                ...
                {isOpen && <div className="col-6 d-flex align-items-center justify-content-end text-secondary">
                    ...
                    >
                        ...
                        {
                            Object.keys(sql_contact.results[0]).map((col) => (
                    ...
                    <CSVLink data={data} filename="Contact_Shagun_Mohta.csv">
                        <button
                            className="btn small"
